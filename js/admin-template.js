export const ADMIN_PANEL_HTML = `<section id="admin-view" class="admin-view admin-app">
      ...
      <nav id="admin-menu" class="admin-menu app-sidebar" aria-label="Menú de administración">
        ...
        <button id="mobile-more-button" class="admin-menu-item mobile-more-button" type="button" aria-haspopup="dialog" aria-controls="mobile-more-sheet"><span>☰</span><strong>Menú</strong><small>Más opciones</small></button>
      </nav>
      <div id="mobile-more-sheet" class="mobile-more-sheet" hidden>
        ...
        <section class="mobile-more-card" role="dialog" aria-modal="true" aria-label="Menú">
          <header><strong>Menú</strong><button type="button" class="icon-button" data-close-more aria-label="Cerrar">×</button></header>
      ...
      <section id="catalog-settings-panel" data-panel-section class="catalog-settings" hidden>
        ...
        <form id="catalog-settings-form">
          ...
          <div class="pdf-generator-card">
            <div class="pdf-generator-heading"><div><p class="eyebrow">Catálogo PDF</p><h3>Generar catálogo para compartir</h3><p>Usa los productos, fotografías y datos actuales del catálogo público. El archivo se genera en este navegador y no se guarda en Supabase.</p></div><span class="pdf-generator-icon" aria-hidden="true">PDF</span></div>
            <form id="catalog-pdf-form" class="pdf-generator-form">
              <div class="pdf-generator-options">
                <label class="setting-check"><input id="pdf-include-sku" type="checkbox" checked><span><strong>SKU</strong><small>Mostrar el código de cada producto.</small></span></label>
                <label class="setting-check"><input id="pdf-include-description" type="checkbox" checked><span><strong>Descripción</strong><small>Incluir la descripción comercial.</small></span></label>
                <label class="setting-check"><input id="pdf-include-detail" type="checkbox" checked><span><strong>Incluye / presentación</strong><small>Mostrar el contenido o detalle distintivo.</small></span></label>
                <label class="setting-check"><input id="pdf-include-price" type="checkbox"><span><strong>Precio minorista</strong><small>Desactivado por defecto para compartir externamente.</small></span></label>
                <label class="setting-check"><input id="pdf-include-stock" type="checkbox" checked><span><strong>Disponibilidad</strong><small>Mostrar “Disponible” o “Agotado”.</small></span></label>
                <label class="setting-check"><input id="pdf-include-out-of-stock" type="checkbox"><span><strong>Incluir agotados</strong><small>Si se activa, también aparecen productos sin stock.</small></span></label>
              </div>
              <div class="pdf-generator-actions"><button id="generate-catalog-pdf" class="button primary" type="submit">Generar catálogo PDF</button><p id="catalog-pdf-message" class="form-message" aria-live="polite"></p></div>
            </form>
          </div>
