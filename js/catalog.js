...
const normalizeCategory = value => {
  ...
  return category === 'Dama' ? 'Mujer' : category === 'Caballero' ? 'Hombre' : category;
};
    ...
    content.innerHTML = `<div class="dialog-gallery">${galleryMarkup(items, activeVisualIndex, displayName)}</div>
      <div class="dialog-details">
        <div class="first-screen-summary">
          <div class="dialog-product-heading"><p class="eyebrow">${escapeHtml(normalizeCategory(product.categoria))}</p><h2>${escapeHtml(displayName)}</h2>${selectedPrice}</div>
        ...
        <section class="dialog-description"><details open><summary>Descripción</summary><div class="rich-display">${sanitizeRichHtml(product.descripcion)}</div></details>${selected.detalle_distintivo ? `<details><summary>Características de esta presentación</summary><div class="variant-detail rich-display">${sanitizeRichHtml(selected.detalle_distintivo)}</div></details>` : ''}</section>
